...
public class BoardDAO {
	...
	public List selectAllArticles(Map pagingMap){
		...
		try{
		   ...
		   String query = "SELECT * FROM (select ROW_NUMBER() OVER(ORDER BY articleNO DESC) as recNum, "
		   		+ "articleNO, title, id, writedate	FROM t_board WHERE articleNO) c "
		   		+ " where c.recNum between(?-1)*100+(?-1)*10+1 and (?-1)*100+?*10";
	  ...
	  return articlesList; /*왜 여기서 안넘어가는겨*/
    } 
	...
	public ArticleVO selectArticle(int articleNO) {
		...
		try {
			conn = dataFactory.getConnection();
			String query = "select articleNO,title,content, imageFileName,id,writeDate" + " from t_board"
					+ " where articleNO=?";
			System.out.println(query);
			pstmt = conn.prepareStatement(query);
			pstmt.setInt(1, articleNO);
